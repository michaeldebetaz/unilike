...
func main() {
	...
	for _, faculty := range faculties {
		facultyHtml, err := scrapper.GetHtml(faculty.Url)
		if err != nil {
			log.Fatalf("Failed to get faculty page html: %v", err)
		}

		programs, err := parser.ExtractPrograms(faculty, facultyHtml)
		if err != nil {
			log.Fatalf("Failed to extract programs: %v", err)
		}

		for _, program := range programs {
			programHtml, err := scrapper.GetHtml(program.Url)
			if err != nil {
				log.Fatalf("Failed to get program page html: %v", err)
			}

			// if err := writer.ToFile(program.FileName+".html", programHtml); err != nil {
